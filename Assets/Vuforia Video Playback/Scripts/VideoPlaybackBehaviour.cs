...
public class VideoPlaybackBehaviour : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            int pos = transform.parent.name.IndexOf("_");
            string[] name = new string[] { transform.parent.name.Substring(0, pos).ToLower(), transform.parent.name.Substring(pos + 1) };

            var regex = new Regex(Regex.Escape(name[0][0].ToString()));
            m_path = "Artifacts/" + regex.Replace(name[0], name[0][0].ToString().ToUpper(), 1) + "/" + name[1] + ".mp4";
            Debug.Log(m_path);
        }
