...
public class TrackableEventHandler : MonoBehaviour, ITrackableEventHandler
{
    ...
    {
        if (PlayerPrefs.GetInt("VideoVisible") == 1)
        {
        ...
        int pos = mTrackableBehaviour.name.IndexOf("_");
        string[] name = new string[] { mTrackableBehaviour.name.Substring(0, pos).ToLower(), mTrackableBehaviour.name.Substring(pos + 1) };

        if (Camera.allCamerasCount > 0)
        {
            Debug.Log(mTrackableBehaviour.name);
            PlayerPrefs.SetString("ImageTarget", mTrackableBehaviour.name);
            discovery.loadArtifactResources(name[0], name[1]);
        }
    ...
    private void OnTrackingLost()
    {
        ...
        {
            Debug.Log(mTrackableBehaviour.name);
            discovery.showArtifactProps(false);
        }
    ...
    public void PauseOtherVideos(VideoPlaybackBehaviour currentVideo)
    {
